...
def _flags_table(flags: dict) -> dict | None:
    """Strengths / Watch / Concerns grouped table (reuses category marker rows)."""
    groups = [("✓ Strengths", "positive"), ("▲ Watch items", "watch"),
              ("▼ Concerns", "concern")]
    rows = []
    for label, key in groups:
        ...
        if not items:
            continue
        rows.append({"_category": f"{label} ({len(items)})"})
        for f in items:
            rows.append([f"{f['title']} ({f['category']})", f["detail"]])
    return _table(["Signal", "What it means"], rows) if rows else None
...
def _build_blocks(state: AnalysisState) -> list:
    ...
    for t in tickers:
        ...
        # Strengths & Concerns scorecard (deterministic flags) — leads the section
        tflags = flags.get(t)
        if tflags:
            ft = _flags_table(tflags)
            if ft:
                blocks.append(_h(3, f"{t}: Strengths & Concerns"))
                s = tflags.get("summary", {})
                blocks.append(_summary(
                    f"Overall: {s.get('tilt', '')}  —  {s.get('line', '')}"))
                blocks.append(ft)
                blocks += _section_text(sections, f"flags_{t}")
    ...
    blocks.append(_p(
        ...
        "(valuation, growth, capital allocation) → Flags (rule-based strengths "
        "& concerns) → Chart Builder → Analyst "
